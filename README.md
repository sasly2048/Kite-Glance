...
- **Native material** — DWM acrylic corners, dark frame, and shadow via `DwmSetWindowAttribute`, plus a pre-rendered mesh-gradient backdrop.
...
1. Go to the [Releases](https://github.com/sasly2048/KiteGlance/releases) page.
...
```powershell
.\scripts\install.ps1              # to install
...
- **Environment variables:** useful when running from source. See the table below.
...
| `KITEGLANCE_DEBUG` | Set to `1` to dump raw API responses to `%APPDATA%\KiteGlance\api-dump.json` |
| `KITEGLANCE_PUBLISHER` | Publisher name used by `install.ps1` for the Start Menu shortcut |
...
```
src/KiteGlance/
├── App.xaml / App.xaml.cs          — Application entry, global error handling
├── MainWindow.xaml / .cs           — Widget UI, layout, keyboard shortcuts
├── Assets/
│   ├── app.ico                     — Tray and window icon
│   ├── backdrop.png                — Pre-rendered mesh-gradient backdrop
│   └── grain.png                   — Grain texture overlay
├── Models/                         — Data models (Holding, PortfolioSummary)
├── Services/
│   ├── KiteApiService.cs           — Kite Connect REST client
│   ├── AmfiNavService.cs           — AMFI NAV fetcher and cache
│   ├── CredentialVault.cs          — DPAPI encrypt/decrypt
│   ├── OAuthServer.cs              — Loopback TcpListener for OAuth
│   ├── PnlMath.cs                  — Pure P&L arithmetic (shared, tested)
│   └── Log.cs                      — Rotating file logger
├── ViewModels/                     — MVVM view models
└── Helpers/                        — Win32 interop, spring easing, utilities

tests/KiteGlance.Tests/            — xUnit tests (pure net8.0, no WPF)
scripts/
├── build.ps1                       — Publish single-file exe (ARM64/x64)
├── install.ps1                     — Per-user install with shortcuts + autostart
└── preflight.py                    — Pre-PR XAML resource validator
.github/workflows/
├── ci.yml                          — Build + test on push/PR (ARM64 + x64 matrix)
└── release.yml                     — Publish binaries on v* tag push
```
...
This software is provided "as is", without warranty of any kind. It is a read-only viewer for your own portfolio and places no trades. Market data may be delayed or inaccurate; **do not rely on it for trading decisions.** Always verify figures against the official Kite and Coin apps.

You are responsible for complying with Zerodha's API terms of use.
